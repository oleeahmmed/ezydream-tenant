...
const ITEMS_API = "/api/inventory/items";

export type ItemSearchModalProps = {
  open: boolean;
  onClose: () => void;
  /** Called when user confirms a row (double-click or OK). */
  onPick: (row: Row) => void;
};

/** Item chooser popup — behaviour aligned with ``frontend/ui/sap2.html`` (#itemSearchModal). */
export function ItemSearchModal({ open, onClose, onPick }: ItemSearchModalProps) {
  ...
  const load = useCallback(async (q: string) => {
    ...
    try {
      ...
      const data = await apiFetch<ListPage>(`${ITEMS_API}?${sp}`);
      setRows(data.items || []);
      setSel(null);
    } catch (e) {
  ...
  return (
    <div className="sap-modal-overlay" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sap-modal-box sap-item-search-modal" role="dialog" aria-labelledby="item-search-title" onMouseDown={(e) => e.stopPropagation()}>
        <div className="sap-modal-head">
          <span id="item-search-title">Item List</span>
          <button type="button" className="sap-modal-close" onClick={onClose} aria-label="Close">
            ✕
          </button>
        </div>
        <div className="sap-modal-toolbar">
          <label className="sap-modal-label" htmlFor="item-search-filter">
            Filter
          </label>
          <input
            ...
            style={{ width: 220 }}
            placeholder="Item no., name, barcode…"
            value={filter}
          ...
          <table className="sap-table item-search-table">
            ...
            <tbody>
              {rows.map((r, i) => (
                ...
                >
                  <td>{r.ItemCode != null ? String(r.ItemCode) : ""}</td>
                  <td>{r.ItemName != null ? String(r.ItemName) : ""}</td>
                  <td>{r.OnHand != null ? String(r.OnHand) : ""}</td>
